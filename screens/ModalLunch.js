...
const ModalLunch = ({toModalLunch}) => {
  ...
  const renderBackAction = () => (
    <TopNavigationAction
      icon={CloseIcon}
      onPress={() => toModalLunch.setLunchVisible(false)}
    />
  );
  ...
  const Fait = async () => {
    ...
    axios.post(`${BASE_URL}/wp-json/repas/ulunchfait`, ulunchfait)
      .then(getRepasFait(ftodayDate))
      .catch(err => {console.log(err.response.data.message)})
      .finally(() => toModalLunch.setLunchVisible(false))
  }
  const Delete = async () => {
    const ulunchfait ={
      lunchfait: 0,
  ...
  if(isLoaded) {
    return (
      <SafeAreaView style={styles.ModalContainer}>
        ...
        <TopNavigation style={styles.ModalTopContainer} title={LunchTitleModal} accessoryLeft={renderBackAction}/>
  ...
  return (
    <SafeAreaView style={styles.ModalContainer}>
      ...
      <Layout style={styles.bottom} level='1'>
        <Button style={{width: windowWidth-50}} disabled={NotToday} size={'large'} onPress={LnFait ? Delete : Fait} accessoryRight={LnFait && DeleteIcon}>Fait</Button>
      </Layout>
    </SafeAreaView>
  )
}

export default ModalLunch
